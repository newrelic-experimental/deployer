...
* Use this command to create an RSA SSH key pair with a length of 4096 bits `ssh-keygen -m PEM -t rsa -b 4096 -f $HOME/deployer/configs/azrkey --N "" -C "compute-user"`. If you would like to create the key in a directory other than `HOME/deployer/configs`, change the file path after the `-f` option. 
